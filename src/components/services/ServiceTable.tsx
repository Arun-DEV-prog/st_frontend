...
export default function ServiceTable() {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      form.append('duration_days', String(editData.duration_days));
      form.append('base_price', String(editData.base_price));
      form.append('service_category', editData.service_category || '');
      form.append('is_draft', editData.is_draft ? 'true' : 'false');
      // Images
      (editData.media || []).forEach((m: any, idx: number) => {
        if (m.file) form.append(`image${idx+1}`, m.file);
      });
      await axios.put(`http://localhost:5000/api/specialists/${editId}`, form, {
            ...
            {editLoading ? (
              ...
              <>
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded" disabled={editLoading}>{editLoading ? 'Publishing...' : 'Published'}</button>
                </div>
              </>
            ) : (
              <div>No data</div>
